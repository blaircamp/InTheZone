...
struct LogEntry {
    ...
    func formatted() -> String {
        ...
        var logLine = "[\(timestampStr)] [\(level.rawValue)] [WATCH] [\(source)] \(message)"
...
final class SessionLogger: ObservableObject {
    ...
    private let fileQueue = DispatchQueue(label: "watchSessionLogger.fileQueue", qos: .utility)
    private var currentLogFile: FileHandle?
    private var currentLogURL: URL?
    private let logger = Logger(subsystem: "com.inTheZone.debug.watch", category: "SessionLogger")
    ...
    private func setupLogDirectory() {
        ...
        let logsDirectory = documentsURL.appendingPathComponent("WatchSessionLogs")
        
        if !FileManager.default.fileExists(atPath: logsDirectory.path) {
            do {
                try FileManager.default.createDirectory(at: logsDirectory, withIntermediateDirectories: true)
                logger.info("Created watch logs directory at \(logsDirectory.path)")
            } catch {
                logger.error("Failed to create watch logs directory: \(error.localizedDescription)")
            }
        }
    }
            ...
            for fileURL in logFiles {
                if let creationDate = try fileURL.resourceValues(forKeys: [.creationDateKey]).creationDate,
                   creationDate < cutoffDate {
                    ...
                    logger.info("Deleted old watch log file: \(fileURL.lastPathComponent)")
                }
            }
        } catch {
            logger.error("Failed to cleanup old watch logs: \(error.localizedDescription)")
        }
    }
    ...
    private func createSessionLogFile() {
        ...
        let filename = "watch_session_\(timestamp).log"
        
        guard let documentsURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first else {
            logger.error("Failed to get documents directory for watch log file creation")
            return
        }
        ...
        do {
            ...
            logger.info("Created watch session log file: \(filename)")
            
            // Write header
            let header = "=== InTheZone Watch Session Debug Log ===\n"
            writeToFile(header)
            
        } catch {
            logger.error("Failed to create watch log file: \(error.localizedDescription)")
        }
    }
    ...
    private func writeSystemInfo() {
        ...
        log(level: .info, source: "WatchSystemInfo", message: "Watch session started", metadata: systemInfo)
    }
    ...
    private func closeCurrentLogFile() {
        if let logFile = currentLogFile {
            let footer = "\n=== Watch Session Debug Log End ===\n"
            writeToFile(footer)
            
            do {
                try logFile.close()
                logger.info("Closed watch session log file")
            } catch {
                logger.error("Failed to close watch log file: \(error.localizedDescription)")
            }
        }
    ...
    private func writeToFile(_ content: String) {
        ...
        do {
            ...
            logger.error("Failed to write to watch log file: \(error.localizedDescription)")
        }
    }
    ...
    func getAllLogFiles() -> [URL] {
        ...
        do {
            ...
            logger.error("Failed to get watch log files: \(error.localizedDescription)")
            return []
        }
    }
}
